...
async def ensure_group(r: redis.Redis):
    ...
    try:
        ...
        pass
    # ensure delayed zset exists (noop: zadd will create)

async def enqueue_job(r: redis.Redis, job_id: str, payload: Dict[str, Any]) -> Tuple[str, str]:
    """Enqueue a job. If job already exists (idempotency), do not enqueue again.

    Returns (mode, msg_id):
      mode = "ENQUEUED" | "EXISTS"
    """
    now = int(time.time())
...
async def schedule_retry(r: redis.Redis, job_id: str, due_ts: float, reason: str):
    """Schedule a retry by placing job_id in a ZSET with score=due_ts."""
...
async def pop_due_retry(r: redis.Redis, now_ts: float) -> str | None:
    """Pop one due retry if score <= now_ts. Uses ZPOPMIN then checks due."""
