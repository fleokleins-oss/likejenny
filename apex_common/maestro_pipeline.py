...
async def run_pipeline(
    *,
    ...
    shadow = await _get_json(
        http,
        f"{shadow_url}/get_market_state/{shadow_symbol}",
        limiter=lim_shadow,
        timeout=timeout_s,
        attempts=attempts,
    )
    ...
    notes.append(f"final_risk_pct={final_risk_pct:.5f} (base={base_risk_pct:.5f}, risk_mult={risk_mult:.2f}, conf={conf:.2f})")
    ...
    if bool(req.get("dry_run", False)):
        ...
        resp["execution"] = {
            ...
            "mapped_side": "buy" if decision.get("side") == "LONG" else "sell" if decision.get("side") == "SHORT" else "none",
    ...
    result = await _post_json(
        http,
        f"{exec_url}/execute_strike",
        exec_payload,
        limiter=lim_exec,
        timeout=max(timeout_s, 8.0),
        attempts=attempts,
    )
    resp["execution"] = result
    resp["status"] = "EXECUTED" if result.get("status") == "SUCCESS" else result.get("status") or "EXECUTED"
    return resp
